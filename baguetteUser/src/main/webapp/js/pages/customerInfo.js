...
$( '#purchaseDatepicker' ).on('change', function(){
	//console.log('test');
	 $('#cdiv').html("");
	 $.ajax({
	        // 회원 번호를 통해 호출
	        url: COMMONWEBSERVER + "/border/listBorder/" + LOGIN_NO + "?requestDate="+$( '#purchaseDatepicker' ).val(),
	        method: "GET",
	        dataType: "json",
	        headers: {
	            "Accept": "application/json",
	        ...
	        success: function (JSONData, status) {
	            $.each(JSONData.map.list, function (i, item) {
	                ...
	                var ctexttotal = $(document.createElement('div')).addClass('ctotal').attr('id', 'ctotal'+cindex);
	                ...
	                $('#cdiv').append(cdiv.html(cimgdiv.html(cimg)).append(ctexttotal));
	                $('#ctotal'+cindex).append(ctexttop).append(ctextmid).append(ctextbot).append(ctextbot2).append(ctextbot3);
	                $('#ctop' + cindex).append("이름 : ").append(JSONData.map.list[i].name);
	                ...
	                $('#cbot3' + cindex).append("구매가격 : ").append(JSONData.map.list[i].price * JSONData.map.list[i].oqty).append("원");
	            })
	        }
	    });
});